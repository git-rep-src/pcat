    ...
    const struct lstr test_names[], const struct lstr expected[])
{
    ...
    pcat_assert(gen_cert(&cert, &key, commonNames, dNSNames) == 1);

    pcat_assert(BIO_new_bio_pair(&server_bio, 0, &client_bio, 0) == 1);

    server_ctx = SSL_CTX_new(SSLv23_server_method());
    pcat_assert(server_ctx != NULL);

    client_ctx = SSL_CTX_new(SSLv23_client_method());
    pcat_assert(client_ctx != NULL);
    ...
    pcat_assert(server_ssl != NULL);
    SSL_set_accept_state(server_ssl);
    SSL_set_bio(server_ssl, server_bio, server_bio);
    pcat_assert(SSL_use_certificate(server_ssl, cert) == 1);
    pcat_assert(SSL_use_PrivateKey(server_ssl, key) == 1);

    client_ssl = SSL_new(client_ctx);
    pcat_assert(client_ssl != NULL);
