...
# This file contains tests of the external behavior of Pcat.
...
my $PORT = 40000;
my $PROXY_PORT = 40001;
my $UNIXSOCK = "pcat.unixsock";
my $UNIXSOCK_TMP = "pcat.unixsock_tmp";
...
# Run $PCAT with the given arguments.
sub pcat {
	my $pid;
	local *IN;
	local *OUT;
...
sub pcat_server {
	my @ret = pcat($PORT, "--test", "-l", @_);
	wait_listen($ret[3]);
	return @ret;
}

sub pcat_server_noport {
	my @ret = pcat("--test", "-l", @_);
	wait_listen($ret[3]);
	return @ret;
}
...
sub pcat_client {
	my $host;
	my @ret = pcat(host_for_args(@_), $PORT, @_);
...
sub server_client_test {
	...
	my $code = shift;
	($s_pid, $s_out, $s_in) = pcat_server(@$server_args);
	($c_pid, $c_out, $c_in) = pcat_client(@$client_args);
	test($desc, $code);
	kill_children;
}
...
sub proxy_test {
	...
	my $client_args = shift;
	my $code = shift;
	($p_pid, $p_out, $p_in) = pcat(host_for_args(@$proxy_args), ($PROXY_PORT, "-l", "--proxy-type", "http"), @$proxy_args);
	($s_pid, $s_out, $s_in) = pcat(host_for_args(@$server_args), ($PORT, "-l"), @$server_args);
	($c_pid, $c_out, $c_in) = pcat(host_for_args(@$client_args), ($PORT, "--proxy", "$HOST:$PROXY_PORT"), @$client_args);
	test($desc, $code);
	kill_children;
}
...
sub proxy_test_raw {
	...
	($p_pid, $p_out, $p_in) = pcat(host_for_args(@$proxy_args), ($PROXY_PORT, "-l", "--proxy-type", "http"), @$proxy_args);
	($s_pid, $s_out, $s_in) = pcat(host_for_args(@$server_args), ($PORT, "-l"), @$server_args);
	($c_pid, $c_out, $c_in) = pcat(host_for_args(@$client_args), ($PROXY_PORT), @$client_args);
	test($desc, $code);
	kill_children;
}
...
sub max_conns_test {
	...
	($s_pid, $s_out, $s_in) = pcat_server(@$server_args, ("--max-conns", $count));
	test $desc, sub {
		my ($i, $resp);
...
sub match_pcat_environment {
	$_ = shift;
	return /PCAT_REMOTE_ADDR=.+\n
			PCAT_REMOTE_PORT=.+\n
			PCAT_LOCAL_ADDR=.+\n
			PCAT_LOCAL_PORT=.+\n
			PCAT_PROTO=.+
...
($s_pid, $s_out, $s_in) = pcat_server_noport();
test "Server default listen address and port",
sub {
	my $resp;

	my ($c_pid, $c_out, $c_in) = pcat("127.0.0.1");
	syswrite($c_in, "abc\n");
	close $c_in;
...
sub {
	...
	my ($c_pid2, $c_out2, $c_in2) = pcat("-6", "::1");
	syswrite($c_in2, "abc\n");
	close $c_in2;
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\"";
...
($s_pid, $s_out, $s_in) = pcat_server_noport("-4");
...
($s_pid, $s_out, $s_in) = pcat_server_noport("-6");
test "Server -6 default listen address and port",
sub {
	my $resp;

	my ($c_pid, $c_out, $c_in) = pcat("-6", $IPV6_ADDR);
	syswrite($c_in, "abc\n");
	close $c_in;
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\"";
...
($s_pid, $s_out, $s_in) = pcat_server_noport("-l", $HOST);
test "Server default port",
sub {
	my $resp;

	my ($c_pid, $c_out, $c_in) = pcat($HOST);
	syswrite($c_in, "abc\n");
	close $c_in;
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\"";
...
($s_pid, $s_out, $s_in) = pcat_server();
test "Server default listen address",
sub {
	my $resp;

	my ($c_pid, $c_out, $c_in) = pcat($HOST, $PORT);
	syswrite($c_in, "abc\n");
	close $c_in;
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\"";
...
($s_pid, $s_out, $s_in) = pcat_server_noport("--udp", "-4");
test "Server default listen address --udp IPV4",
sub {
	my $resp;

	my ($c_pid, $c_out, $c_in) = pcat("localhost", "--udp");
	syswrite($c_in, "abc\n");
	close $c_in;
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\" from localhost";
...
($s_pid, $s_out, $s_in) = pcat_server_noport("--udp", "-6");
test "Server default listen address --udp IPV6",
sub {
	my $resp;

	my ($c_pid1, $c_out1, $c_in1) = pcat("localhost", "--udp");
	syswrite($c_in1, "abc\n");
	close $c_in1;
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\" from ::1";
...
{
local $xfail = 1;
($s_pid, $s_out, $s_in) = pcat_server_noport("--udp");
...
sub {
  my $resp;
	...
	my ($c_pid1, $c_out1, $c_in1) = pcat("::1", "--udp");
	syswrite($c_in1, "abc\n");
	close $c_in1;
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\" from ::1";
...
($s_pid, $s_out, $s_in) = pcat_server_noport("-6", "--udp");
test "Server default listen address -6 --udp",
sub {
	my $resp;

	my ($c_pid, $c_out, $c_in) = pcat("127.0.0.1", "--udp");
	syswrite($c_in, "abc\n");
	close $c_in;
	$resp = timeout_read($s_out);
...
($s_pid, $s_out, $s_in) = pcat_server_noport("-4", "--udp");
...
sub {
	...
	my ($c_pid, $c_out, $c_in) = pcat("localhost");
	syswrite($c_in, "abc\n");
	close $c_in;
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\"";
...
($s_pid, $s_out, $s_in) = pcat_server_noport("-U", $UNIXSOCK);
test "Server UNIX socket listen on $UNIXSOCK (STREAM)",
sub {
	my $resp;

	my ($c_pid, $c_out, $c_in) = pcat("-U", $UNIXSOCK);
	syswrite($c_in, "abc\n");
	close $c_in;
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\" from client";
...
{
local $xfail = 1 if !$HAVE_UNIXSOCK;
($s_pid, $s_out, $s_in) = pcat_server_noport("-U", "--udp", $UNIXSOCK);
test "Server UNIX socket listen on $UNIXSOCK --udp (DGRAM)",
sub {
	my $resp;

	my ($c_pid, $c_out, $c_in) = pcat("-U", "--udp", $UNIXSOCK);
	syswrite($c_in, "abc\n");
	close $c_in;
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\" from client";
...
sub {
	...
	my ($c_pid, $c_out, $c_in) = pcat($HOST, $PORT, "--send-only");
	syswrite($c_in, "abc\n");
	close($c_in);
	do {
		$pid = waitpid($c_pid, 0);
...
sub {
	...
	my ($s_pid, $s_out, $s_in) = pcat_server("-4");
...
sub {
	...
	my ($c_pid, $c_out, $c_in) = pcat($HOST, $PORT, "--baffle");
	do {
		$pid = waitpid($c_pid, 0);
	} while ($pid > 0 && $pid != $c_pid);
	$pid == $c_pid or die;
	...
	my ($s_pid, $s_out, $s_in) = pcat("-l", "--baffle");
	do {
		$pid = waitpid($s_pid, 0);
	} while ($pid > 0 && $pid != $s_pid);
	$pid == $s_pid or die;
...
#   pcat -l | tar xzvf -
#   tar czf - <files> | pcat localhost --send-only
...
sub {
	...
	my ($c_pid, $c_out, $c_in) = pcat_client();
	syswrite($c_in, "abc\n");
	$resp = timeout_read($s_out);
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\"";
	kill "TERM", $c_pid;
...
($s_pid, $s_out, $s_in) = pcat_server("--exec", "$CAT");
...
{
($s_pid, $s_out, $s_in) = pcat_server("--ssl");
...
sub {
	my ($c1_pid, $c1_out, $c1_in) = pcat_client();
	my ($c2_pid, $c2_out, $c2_in) = pcat_client();
...
($s_pid, $s_out, $s_in) = pcat_server("--keep-open");
test "--keep-open",
sub {
	my $resp;
...
($s_pid, $s_out, $s_in) = pcat_server("--keep-open", "--exec", "$CAT");
test "--keep-open --exec",
sub {
	my $resp;
...
($s_pid, $s_out, $s_in) = pcat_server("--keep-open", "--udp", "--exec", "$CAT");
test "--keep-open --exec (udp)",
sub {
	my $resp;

	my ($c1_pid, $c1_out, $c1_in) = pcat_client("--udp");
	syswrite($c1_in, "abc\n");
	$resp = timeout_read($c1_out);
	$resp eq "abc\n" or die "Client 1 got back \"$resp\", not \"abc\\n\"";

	my ($c2_pid, $c2_out, $c2_in) = pcat_client("--udp");
	syswrite($c2_in, "abc\n");
	$resp = timeout_read($c2_out);
	$resp eq "abc\n" or die "Client 2 got back \"$resp\", not \"abc\\n\"";
};
...
["--exec", "$BINSH test-environment.sh"], [], sub {
	syswrite($c_in, "abc\n");
	my $resp = timeout_read($c_out) or die "Read timeout";
	match_pcat_environment($resp) or die "Client received " . d($resp) . ".";
};
...
($s_pid, $s_out, $s_in) = pcat_server("--broker", "--send-only");
...
($s_pid, $s_out, $s_in) = pcat_server("--broker", "--recv-only");
...
#Broker Tests
($s_pid, $s_out, $s_in) = pcat_server("--broker");
...
($s_pid, $s_out, $s_in) = pcat_server("--broker", "--sctp");
test "--broker mode (sctp)",
sub {
	my $resp;

	my ($c1_pid, $c1_out, $c1_in) = pcat_client("--sctp");
	my ($c2_pid, $c2_out, $c2_in) = pcat_client("--sctp");
...
($s_pid, $s_out, $s_in) = pcat_server("--broker", "--ssl");
test "--broker mode (tcp ssl)",
sub {
	my $resp;

	my ($c1_pid, $c1_out, $c1_in) = pcat_client("--ssl");
	my ($c2_pid, $c2_out, $c2_in) = pcat_client("--ssl");
...
{
  local $xfail=1 if !$HAVE_SCTP;
($s_pid, $s_out, $s_in) = pcat_server("--broker", "--sctp", "--ssl");
test "--broker mode (sctp ssl)",
sub {
	my $resp;

	my ($c1_pid, $c1_out, $c1_in) = pcat_client("--sctp", "--ssl");
	my ($c2_pid, $c2_out, $c2_in) = pcat_client("--sctp", "--ssl");
...
sub {
	my $resp;
	sleep 1;
	my ($c1_pid, $c1_out, $c1_in) = pcat_client("-6");
	my ($c2_pid, $c2_out, $c2_in) = pcat_client("-4");
...
sub {
	...
	my ($c_pid, $c_out, $c_in) = pcat("-p", "1234", $HOST, $PORT);
...
sub {
	...
	my ($c_pid, $c_out, $c_in) = pcat("--proxy-type", "http", "--proxy", "$HOST:$PROXY_PORT", "-p", "1234", $HOST, $PORT);
...
sub {
	...
	my ($c_pid, $c_out, $c_in) = pcat("--proxy-type", "socks4", "--proxy", "$HOST:$PROXY_PORT", "-p", "1234", $HOST, $PORT);
...
sub {
	...
	my ($c_pid, $c_out, $c_in) = pcat("-U", "--udp", "-s", $UNIXSOCK_TMP, $UNIXSOCK);
	syswrite($c_in, "abc\n");
	close($c_in);
...
["-k"], ["--proxy", "$HOST:$PORT", "--proxy-type", "http", "--proxy-auth", $auth], sub {
	my $resp = timeout_read($s_out) or die "Read timeout";
	syswrite($s_in, "HTTP/1.0 407 Auth\r\nProxy-Authenticate: Basic realm=\"Pcat\"\r\n\r\n");
	$resp = timeout_read($s_out) or die "Read timeout";
...
($s_pid, $s_out, $s_in) = pcat_server("--proxy-type", "http", "--proxy-auth", "user:pass");
...
my ($fh, $filename) = tempfile("pcat-test-XXXXX", SUFFIX => ".txt");
print $fh $contents;
...
($s_pid, $s_out, $s_in) = pcat_server("--ssl", "--ssl-key", "test-cert.pem", "--ssl-cert", "test-cert.pem", "--keep-open");
...
{
($s_pid, $s_out, $s_in) = pcat_server("--ssl", "--ssl-key", "test-cert.pem", "--ssl-cert", "test-cert.pem");
...
{
($s_pid, $s_out, $s_in) = pcat_server("--ssl", "--exec","$PERL -e \$|=1;while(<>)\{tr/a-z/A-Z/;print\}", "--ssl-key", "test-cert.pem", "--ssl-cert", "test-cert.pem", "--keep-open");
...
sub {
	...
	($c_pid, $c_out, $c_in) = pcat("localhost", $PORT, "--ssl-verify", "--ssl-trustfile", "test-cert.pem");
	syswrite($c_in, "abc\n");
	$resp = timeout_read($s_out);
	$resp or die "Read timeout";
	$resp eq "abc\n" or die "Server got \"$resp\", not \"abc\\n\"";
...
sub {
	...
	# on the certificate.
	($c_pid, $c_out, $c_in) = pcat($IPV6_ADDR, $PORT, "-6", "--ssl-verify", "--ssl-trustfile", "test-cert.pem");
	syswrite($c_in, "abc\n");
	$resp = timeout_read($s_out);
...
# Test --max-conns.
($s_pid, $s_out, $s_in) = pcat_server("--keep-open", "--max-conns", "1");
...
($s_pid, $s_out, $s_in) = pcat_server("--broker", "--max-conns", "1");
...
($s_pid, $s_out, $s_in) = pcat_server("--ssl", "--ssl-key", "test-cert.pem", "--ssl-cert", "test-cert.pem", "--keep-open", "--max-conns", "1");
...
($s_pid, $s_out, $s_in) = pcat_server("--ssl", "--ssl-key", "test-cert.pem", "--ssl-cert", "test-cert.pem", "--broker", "--max-conns", "1");
...
sub socks5_auth {
    ...
    my ($c_pid, $c_out, $c_in) = pcat("--proxy-type", "socks5", "--proxy", "localhost:$PROXY_PORT", @$auth_data, $ipvx, $dest_addr, $PORT);
...
sub {
    ...
    my ($c_pid, $c_out, $c_in) = pcat("-4","--proxy-type", "socks5", "--proxy", "$HOST:$PROXY_PORT", "127.0.0.1", $PORT);
...
sub {
        ...
        my ($c_pid, $c_out, $c_in) = pcat_client("-z");
...
($s_pid, $s_out, $s_in) = pcat_server("--udp");
test "-z client sends \"\\0\" to server and exits with success exit code (udp)",
sub {
        my ($resp, $pid, $code);
        local $SIG{CHLD} = sub { };

        my ($c_pid, $c_out, $c_in) = pcat_client("-z", "--udp");
        $resp = timeout_read($s_out);
...
sub {
        my ($pid, $code);
        local $SIG{CHLD} = sub { };
        ($c_pid, $c_out, $c_in) = pcat_client("-z");
...
sub {
        my ($pid, $code);
        local $SIG{CHLD} = sub { };
        ($c_pid, $c_out, $c_in) = pcat_client("-z", "--udp");
