...
xml "OPEN" "DATA type='show' name='pe' namemax='16'"
if ($PSV -ne "" -and $PSV -ne $null -and $PSV -gt 2 -and $? -eq "True") {
    xml "OPEN" "FILES"
    foreach ($FOLDER in $LIST_FOLDERS) {
        $RET = (Test-Path -ErrorAction SilentlyContinue $Env:WINDIR$FOLDER)
    ...
    xml "CLOSE" "FILES"
    xml "OPEN" "PROGRAMS"
    $RET = (Test-Path -ErrorAction SilentlyContinue "Registry::HKEY_CURRENT_USER\SOFTWARE\Policies\Microsoft\Windows\Installer")
    if ($RET -ne "" -and $RET -ne $null -and $? -eq "True") {
