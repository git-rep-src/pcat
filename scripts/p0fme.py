...
about an IP read from PCAT_REMOTE_ADDR environment variable. Then it prints out
all the information it has found. To try it out, run "p0f -i any -o p0f.log"
and pcat -l -k --sh-exec "python p0fme.py".
...
if __name__ == "__main__":
    try:
        ip = os.environ['PCAT_REMOTE_ADDR']
        if os.environ['PCAT_PROTO'] != 'TCP':
            sys.exit("ERROR: This script works for TCP servers only!")
    except KeyError:
        sys.exit("ERROR: This script has to be run from inside of Pcat.")
