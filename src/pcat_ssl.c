/***************************************************************************
 * pcat_ssl.c -- SSL support functions.                                    *
...
#define CERTIFICATE_COMMENT "Automatically generated by Pcat."
...
/* Match a hostname against the contents of the "most specific" commonName field
   ...
   Pcat: wildcard patterns are always less specific than non-wildcard patterns.
...
int ssl_handshake(struct fdinfo *sinfo)
{
    ...
    if (sinfo == NULL) {
        if (o.debug)
           logdebug("pcat_ssl.c: Invoking ssl_handshake() with a NULL parameter "
    ...
    if (ret == 1)
        return PCAT_SSL_HANDSHAKE_COMPLETED;

    sslerr = SSL_get_error(sinfo->ssl, ret);

    if (ret == -1) {
        if (sslerr == SSL_ERROR_WANT_READ)
            return PCAT_SSL_HANDSHAKE_PENDING_READ;
        if (sslerr == SSL_ERROR_WANT_WRITE)
            return PCAT_SSL_HANDSHAKE_PENDING_WRITE;
    }
    ...
    return PCAT_SSL_HANDSHAKE_FAILED;
}
