/***************************************************************************
 * pcat_main.c -- main function: option parsing and checking, dispatching  *
...
static int pcat_connect_mode(void);
static int pcat_listen_mode(void);
        ...
        case 0:
            if (strcmp(long_options[option_index].name, "version") == 0) {
                ...
                /* This command-line switch is undocumented on purpose. Do NOT use it
                   explicitly as its behavior might differ between Pcat releases.

                   Its goal is to switch the Pcat process to the Lua interpreter state
...
"      Windows need a powershell reverse shell.\n"
        ...
        case '?':
            ...
            bye("Try `--help' or man(1) pcat for more information, usage options and help.");
        default:
    ...
    if (o.normlog)
        o.normlogfd = pcat_openlog(o.normlog, o.append);
    if (o.hexlog)
        o.hexlogfd = pcat_openlog(o.hexlog, o.append);
    ...
    if (o.proxyaddr) {
        ...
        if (o.proxytype) {
            ...
            if (strcmp(o.proxytype, "http"))
                bye("Invalid proxy type \"%s\"; Pcat proxy server only supports \"http\".", o.proxytype);
        }
    }

    if (!o.proxy_auth)
        o.proxy_auth = getenv("PCAT_PROXY_AUTH");
...
#endif
        /* Listen defaults to any address and DEFAULT_PCAT_PORT */
        if (!o.listen)
    ...
    if (srcport != -1) {
        if (o.listen) {
            /* Treat "pcat -l -p <port>" the same as "pcat -l <port>" for nc
               compatibility. */
...
connection brokering should work.");
    ...
    if (o.listen)
        return pcat_listen_mode();
    else
        return pcat_connect_mode();
}
