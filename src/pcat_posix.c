/***************************************************************************
 * pcat_posix.c -- POSIX-specific functions.                               *
...
/* Run the given command line as if with exec. What we actually do is fork the
   ...
   the subprocess. This allows Pcat to handle SSL from the socket and give plain
...
void netexec(struct fdinfo *info, char *cmdexec)
{
    ...
    if (pid == 0) {
        ...
        /* We might have turned off SIGPIPE handling in pcat_listen.c. Since
    ...
    for (;;) {
        ...
        if (FD_ISSET(info->fd, &fds)) {
            ...
            do {
                n_r = pcat_recv(info, buf, sizeof(buf), &pending);
                if (n_r <= 0)
                    goto loop_end;
...
int pcat_openlog(const char *logfile, int append)
{
    if (append)
...
#define PCAT_CA_CERTS_PATH (PCAT_DATADIR "/" PCAT_CA_CERTS_FILE)
...
int ssl_load_default_ca_certs(SSL_CTX *ctx)
{
    ...
    rc = SSL_CTX_load_verify_locations(ctx, PCAT_CA_CERTS_PATH, NULL);
    if (rc != 1) {
        if (o.debug)
            logdebug("Unable to load trusted CA certificates from %s: %s\n",
                PCAT_CA_CERTS_PATH, ERR_error_string(ERR_get_error(), NULL));
        return -1;
    }
