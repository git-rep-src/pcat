...
/* Log a user message without the "Pcat: " prefix, to allow building up a line
...
int do_listen(int type, int proto, const union sockaddr_u *srcaddr_u)
{
    ...
    /* We need a socket that can be inherited by child processes in
       pcat_exec_win.c, for --exec and --sh-exec. inheritable_socket is from
       nbase. */
