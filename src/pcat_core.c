/***************************************************************************
 * pcat_core.c -- Contains option definitions and miscellaneous functions. *
    ...
    struct sockaddr_list *sl, int af, int addl_flags, int multiple_addrs)
{
    ...
    pcat_assert(rc >= 0 && (size_t) rc < sizeof(portbuf));
    ...
    if (result == NULL)
        return EAI_NONAME;
    pcat_assert(result->ai_addrlen > 0 && result->ai_addrlen <= (int) sizeof(struct sockaddr_storage));
...
int pcat_recv(struct fdinfo *fdn, char *buf, size_t size, int *pending)
{
    int n;
...
int pcat_send(struct fdinfo *fdn, const char *buf, size_t size)
{
    int n;
...
/* Broadcast a message to all the descriptors in fds. Returns -1 if any of the
   sends failed. */
int pcat_broadcast(fd_set *fds, const fd_list_t *fdlist, const char *msg, size_t size)
{
    struct fdinfo *fdn;
    int i, ret;
 ...
 */
int pcat_delay_timer(int delayval)
{
    struct timeval s;
...
static int pcat_hexdump(int logfd, const char *data, int len);

void pcat_log_send(const char *data, size_t len)
{
    if (o.normlogfd != -1)
        Write(o.normlogfd, data, len);

    if (o.hexlogfd != -1)
        pcat_hexdump(o.hexlogfd, data, len);
}

void pcat_log_recv(const char *data, size_t len)
{
    /* Currently the log formats don't distinguish sends and receives. */
    pcat_log_send(data, len);
}
...
void setup_environment(struct fdinfo *info)
{
    ...
    if (su.sockaddr.sa_family == AF_UNIX) {
        ...
        setenv_portable("PCAT_REMOTE_ADDR", "localhost");
        setenv_portable("PCAT_REMOTE_PORT", "");
    } else
#endif
    ...
    if (su.sockaddr.sa_family == AF_VSOCK) {
        ...
        setenv_portable("PCAT_REMOTE_ADDR", char_u32);

        snprintf(char_u32, sizeof(char_u32), "%u", su.vm.svm_port);
        setenv_portable("PCAT_REMOTE_PORT", char_u32);
    } else
#endif
    if (getnameinfo((struct sockaddr *)&su, alen, ip, sizeof(ip),
            port, sizeof(port), NI_NUMERICHOST | NI_NUMERICSERV) == 0) {
        setenv_portable("PCAT_REMOTE_ADDR", ip);
        setenv_portable("PCAT_REMOTE_PORT", port);
    } else {
    ...
    if (su.sockaddr.sa_family == AF_UNIX) {
        ...
        setenv_portable("PCAT_LOCAL_ADDR", "localhost");
        setenv_portable("PCAT_LOCAL_PORT", "");
    } else
#endif
#ifdef HAVE_LINUX_VM_SOCKETS_H
    if (su.sockaddr.sa_family == AF_VSOCK) {
        ...
        snprintf(char_u32, sizeof(char_u32), "%u", su.vm.svm_cid);
        setenv_portable("PCAT_LOCAL_ADDR", char_u32);

        snprintf(char_u32, sizeof(char_u32), "%u", su.vm.svm_port);
        setenv_portable("PCAT_LOCAL_PORT", char_u32);
    } else
#endif
    if (getnameinfo((struct sockaddr *)&su, alen, ip, sizeof(ip),
            port, sizeof(port), NI_NUMERICHOST | NI_NUMERICSERV) == 0) {
        setenv_portable("PCAT_LOCAL_ADDR", ip);
        setenv_portable("PCAT_LOCAL_PORT", port);
    } else {
        bye("getnameinfo failed: %s", socket_strerror(socket_errno()));
    }
    ...
    switch(o.proto) {
        case IPPROTO_TCP:
            setenv_portable("PCAT_PROTO", "TCP");
            break;
        case IPPROTO_SCTP:
            setenv_portable("PCAT_PROTO", "SCTP");
            break;
        case IPPROTO_UDP:
...
void set_postdir()
{
    ...
    if ((stat("post", &st) == 0) && S_ISDIR(st.st_mode))
        ...
        sprintf(postdir, "%s/post", PCAT_DATADIR);
}
