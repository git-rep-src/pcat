/***************************************************************************
 * pcat_proxy.c -- HTTP proxy server.                                      *
...
int pcat_http_server(void)
{
    ...
    for (i = 0; i < num_listenaddrs; i++) {
        ...
        /* make us not block on accepts in weird cases. See pcat_listen.c:209 */
...
static int send_proxy_authenticate(struct fdinfo *fdn, int stale)
{
    ...
    strbuf_append_str(&buf, &size, &offset, "Proxy-Authenticate: Basic realm=\"Pcat\"\r\n");
#if HAVE_HTTP_DIGEST
    {
        char *hdr;

        hdr = http_digest_proxy_authenticate("Pcat", stale);
    ...
    const struct http_credentials *credentials, int *stale)
{
    ...
    *stale = 0;
    ...
    if (credentials->scheme == AUTH_BASIC) {
        ...
        *password++ = '\0';
        ret = http_digest_check_credentials(username, "Pcat", password,
