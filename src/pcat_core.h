...
/* Broadcast a message to all the descriptors in fds. Returns -1 if any of the
   sends failed. */
extern int pcat_broadcast(fd_set *fds, const fd_list_t *fdlist, const char *msg, size_t size);
 ...
 */
extern int pcat_delay_timer(int delayval);

/* Open a logfile for writing.
 * Return the open file descriptor. */
extern int pcat_openlog(const char *logfile, int append);

extern void pcat_log_send(const char *data, size_t len);

extern void pcat_log_recv(const char *data, size_t len);

extern int pcat_hostaccess(char *matchaddr, char *filename, char *remoteip);

/* Make it so that line endings read from a console are always \n (not \r\n).
   Defined in pcat_posix.c and pcat_win.c. */
